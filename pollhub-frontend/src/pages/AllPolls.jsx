...
export default function AllPolls() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Organiza as enquetes por data de criação (mais recentes primeiro)
      const sortByDate = (a, b) => new Date(b.createdAt) - new Date(a.createdAt);
      setPolls(response.data.sort(sortByDate));
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: error.response?.data?.message || 'Erro ao carregar enquetes',
        severity: 'error'
      });
    } finally {
      setIsLoading(false);
  ...
  const handleCreatePoll = async (pollData) => {
    try {
      ...
      // Adiciona a nova enquete no topo da lista
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      <ContentWrapper component="main">
        ...
        {/* Dialog para exibir detalhes da enquete */}
        <Dialog
          open={!!detalheAberto}
          ...
          PaperProps={{
            sx: {
              ...
              overflowX: 'hidden',
              overflowY: 'auto',
              backgroundColor: 'hsl(220, 30%, 18%)', // igual ao dashboard
            }
          }}
        >
          {/* Título */}
